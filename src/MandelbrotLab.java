...
public class MandelbrotLab {
    ...
    public static double[] multiply(double aReal, double aImag, double bReal, double bImag) {
        ...
        double realPart = aReal * bReal - aImag * bImag; // (a * c) - (b * d)

        // TODO: Calculate the imaginary part of the product (a*d + b*c)
        double imagPart = aReal * bImag + aImag * bReal; // (a * d) + (b * c)
    ...
    public static int countIterations(double cReal, double cImag) {
        ...
        for (int i = 1; i <= MAX_ITERATIONS; i++){ //   Each time through the loop:
            double[] zSquared = multiply(zReal, zImag, zReal, zImag); //1. Use multiply() to compute z² (multiply z by itself)
            double newzReal = zSquared[0] + cReal; //2. Add cReal to the real part of z²  → new zReal
            double newzImag = zSquared[1] + cImag; //Add cImag to the imag part of z²  → new zImag
            if (magnitudeSquared(newzReal, newzImag) > 4.0){ //3. Use magnitudeSquared() to check if z has escaped (> 4.0)
                return i;//4. If it has escaped, return the current loop counter
            }
            zReal = newzReal;
            zImag = newzImag;
        }
        return MAX_ITERATIONS; //If the loop finishes without escaping, return MAX_ITERATIONS.
    }
    // =========================================================================
    ...
    public static double[] pixelToComplex(int px, int py) {
        ...
        double cReal = X_MIN + ((double)px / IMAGE_WIDTH) * (X_MAX - X_MIN);// replace this
